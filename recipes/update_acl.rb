...
namespace :symfony do
  task :update_acl, :roles => :app do
    ...
    run "cd #{latest_release} && setfacl -R -m u:#{webserver_user}:rwX -m u:#{user}:rwX #{dirs.join(" ")}"
    run "cd #{latest_release} && setfacl -dR -m u:#{webserver_user}:rwx -m u:#{user}:rwx #{dirs.join(" ")}"
